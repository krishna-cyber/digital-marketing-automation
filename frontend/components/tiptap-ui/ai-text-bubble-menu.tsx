...
import { TextTransformDialog } from "./text-transform-dialog"
  ...
  return (
    <>
      ...
      {showMenu &&
        ...
        createPortal(
          <div
            className="fixed z-50 rounded border bg-popover text-popover-foreground shadow-lg"
            style={{
              top: position.top,
              left: position.left,
              minWidth: "160px",
          ...
          >
            <div className="flex items-center gap-2 border-b bg-muted/20 px-3 py-1.5 text-sm text-muted-foreground">
            ...
            <div className="space-y-0 py-1">
              {textActions.map((action) => (
                <button
                  key={action.id}
                  className="flex h-8 w-full items-center gap-2 px-3 text-left text-sm hover:bg-muted disabled:opacity-50"
